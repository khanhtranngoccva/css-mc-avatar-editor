...
function _Side(props: SideProps, ref: React.ForwardedRef<HTMLDivElement>) {
    return <div {...props} className={`${classes.side} ${props.className ?? ''}`} ref={ref}>
        {props.children}
    </div>
}
...
function Top(props: SideProps, ref: React.ForwardedRef<HTMLDivElement>) {
    return <Side {...props} className={`${classes.top} ${props.className ?? ''}`} ref={ref}/>;
}

function Bottom(props: SideProps, ref: React.ForwardedRef<HTMLDivElement>) {
    return <Side {...props} className={`${classes.bottom} ${props.className ?? ''}`} ref={ref}/>;
}

function Front(props: SideProps, ref: React.ForwardedRef<HTMLDivElement>) {
    return <Side {...props} className={`${classes.front} ${props.className ?? ''}`} ref={ref}/>;
}

function Back(props: SideProps, ref: React.ForwardedRef<HTMLDivElement>) {
    return <Side {...props} className={`${classes.back} ${props.className ?? ''}`} ref={ref}/>;

}

function Left(props: SideProps, ref: React.Ref<HTMLDivElement>) {
    return <Side {...props} className={`${classes.left} ${props.className ?? ''}`} ref={ref}/>;
}

function Right(props: SideProps, ref: React.Ref<HTMLDivElement>) {
    return <Side {...props} className={`${classes.right} ${props.className ?? ''}`} ref={ref}/>;
}
